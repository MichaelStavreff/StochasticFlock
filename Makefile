...
# STEP 1: Generate profiles
	...
	cp build_pgo/stochastic_flock*.so .
#
	timeout 20s ./build_pgo/bird_solver
...
debug:
	cmake -B build_debug -S . -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_FLAGS="-fsanitize=undefined" -DPython3_EXECUTABLE=$(PYTHON)
	cmake --build build_debug  -j $(THREADS)

clean:
	rm -rf build build_native build_debug build_pgo *.so *.gcda

help:
	@echo "all:    Standard build"
	@echo "native: Fast build with hardware optimizations"
	@echo "pgo:    Slowest build, but maximum performance (requires tuning.py)"