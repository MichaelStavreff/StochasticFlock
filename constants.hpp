...
// inline constexpr double kPD{150*150};             // persistence distance squared, 20
...
inline constexpr int kN_BIRDS{200};
inline constexpr int kM{7};                    // bird set size,        7
inline constexpr double kDELAY{0.1};           // delay,                0.1
inline constexpr double kPT{200};              // persistence time,     700
inline constexpr double kPD{100};              // persistence distance squared, 20^2
inline constexpr double kRT{800};              // refractory time,      800
inline constexpr double kREP{2.5};             // repulsion force,      2.5
inline constexpr double kALI{3};               // alignment force,      3
inline constexpr double kATT{0.01};            // attraction force,     0.01
inline constexpr double kTIMESTEP{0.05};       //                       0.1
inline constexpr double kPROBABILITY{0.00002}; //                       0.0002
inline constexpr double kEPSILON{0.001};       // std::numeric_limits<double>::epsilon() for machine epsilon, 0.0001
inline constexpr int kBUFFER_CYCLES{static_cast<int>(kDELAY / kTIMESTEP)};
